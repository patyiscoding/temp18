...
function open_database_connection() {
    ...
    $connect = new PDO($db_url.';charset=utf8', $db_user, $db_pass);
    return $connect;
}
...
function drawMainSlideshow(){
    ...
    $limit = 7;
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)){
                ...
                $matchAuthor = $authorsArray->fetchAll();

                foreach($matchAuthor as $author) {
                ...
                $result2 .= '<img class="slideshow-img" src="/assets/'.$picturepath.'" alt="Chicago">';
...
function drawPostCategory($id, $categoryName){
    ...
    try{
        ...
        $fetchresult = $result3->fetchAll()[0];
        ...
        $result = $connect->query("SELECT * FROM articles WHERE category = $fetch ORDER BY date");
         while ($row = $result->fetch(PDO::FETCH_ASSOC)){
             if ($row["id"] == $id){
                ...
                $fetchresult2 = $result4->fetchAll()[0];
         ...
         while ($row = $result->fetch(PDO::FETCH_ASSOC)){
             if ($row["id"] == $id){
                ...
                //  $descriptionFeat = $row2['description'];
                 ...
                 $result2 .= "<img src='/assets/".$pictureFeat."'></div>";
                ...
                //  $result2 .= "<p>$descriptionFeat</p>";
                 ...
                 $temp = 0;
                 while ($row3 = $resultSlide->fetch(PDO::FETCH_ASSOC)){
...
function drawArticle($articleURL){
    ...
    try{
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)){
            ...
            foreach($authorArray as $value){
                $authorsArray = $connect->query("SELECT * FROM authors WHERE id = '$value'") or die($connect->error);
                $matchAuthor = $authorsArray->fetchAll()[0];
            ...
            foreach($authorArray as $value){
                ...
                $result2 .= "<img src='/assets/.$author_url'>";
                $result2 .= '</div></div>';
...
//        $result = $connect->query("SELECT * FROM articles WHERE category_name = '$category' ORDER BY date DESC LIMIT 2O") or die($connect->error);
        ...
        $categoryArray = $result3->fetchAll()[0];
...
//        $numResults = mysqli_num_rows($countDivisionRowsResult);
        while ($row = $countDivisionRowsResult->fetch(PDO::FETCH_ASSOC)) {
